...
# Step 5: Export to Excel with Notes
excel_file = "TableS4.xlsx"
notes = [
    "Notes:",
    "- Absorption Rate: Calculated using A(λ) = 1 - T(λ) - R(λ). For CuxO/C PAPS, values are derived from Figure S5 (90–95%); for Carbon Fiber Felt, 85% from previous simulation.",
    ...
    "- Efficiency: Computed using η = (m h_LV)/(C_opt P_0), with dark field evaporation of 0.1 kg·m^-2·h^-1 subtracted.",
    ...
    "- Latent Heat Flux: Computed using Q_evap = - H_evap × m_evap, where H_evap = 2.26 × 10^6 J/kg."
]
notes_df = pd.DataFrame(notes, columns=["Description"])

try:
    with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Table S4")
        notes_df.to_excel(writer, index=False, sheet_name="Notes")
    print(f"Excel file successfully generated as '{excel_file}' with data and notes.")
except Exception as e:
    print(f"Error generating Excel file: {e}")

# Step 6: Plotting Vapor Flow Trends
